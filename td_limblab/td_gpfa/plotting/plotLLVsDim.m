function res = plotLLVsDim(directory,runIdx, varargin)
%
...
% @ 2013 Byron Yu -- byronyu@cmu.edu
  ...
  runDir = fullfile(directory,sprintf('mat_resultsgpfa/run%s', runIdx));
  if ~isdir(runDir)
  ...
  for i = 1:length(D)    
    ...
    if isfield(ws,'LLtest')
        D(i).LLtest    = ws.LLtest;
    else
        % Compute log-likelihood of test data (earlier versions didn't calculate this before
        [~, D(i).LLtest] = exactInferenceWithLL(ws.seqTest, ws.estParams);
    end
