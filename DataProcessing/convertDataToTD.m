            ...
            case 'lfp' % filter into requested bands and downsample
                ...
                for j = 1:length(labels)
                    ...
                    temp_guide = [temp_guide; ...
                        cat(2, repmat(labels(j),size(freq_bands,1),1), ...
                        freq_bands)];
                end
...
if ismember('lfp',file_types)
    ...
    for iSig = 1:length(idx)
        ...
        trial_data.lfp_t   = sig_data(idx(iSig)).t_bin;
    end
end
...
if ischar(which_label{1}) % look for string names in the file list
    ...
    elseif strcmpi(which_type,'lfp')
        if length(file_labels)  == length(which_label)
            idx = 1:length(file_labels);
        else
            idx = 1:length(file_labels);
            warning('LFP labels has not been tested very throughly ...');
        end
    else
